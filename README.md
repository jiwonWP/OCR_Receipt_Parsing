...
**현재 한계**
- 라벨이 완전히 손실된 경우 추출 실패 (예: "총중량" → "총가량")

**개선 방향**
- Fuzzy Matching 도입: Levenshtein Distance 기반으로 유사 라벨 매칭 (예: "총가량" → "총중량" 85% 유사도)


### 2. 차량번호 추출 정확도

**현재 한계**
- `80구8713` 형태만 추출 가능, 신형 번호판 미대응 (`123가4567`, `서울12가3456`)
- digits-only fallback의 낮은 신뢰도 (score=20)

**개선 방향**
- 다양한 번호판 형식 패턴 DB 구축 (일반/신규/지역/특수)
- 컨텍스트 기반 검증: "차량번호" 라벨 존재 여부로 신뢰도 차등 부여
- 추출 성공률 75% → 95% 개선 예상

### 3. 병렬 처리 미지원

**현재 한계**
- 순차 처리로 1000개 파일 처리 시 약 10분 소요
- 멀티코어 환경에서도 단일 코어만 사용

**개선 방향**
- `multiprocessing.Pool`로 병렬 처리 구현
- 4코어 기준 처리 속도 4배 향상 (10분 → 2.5분)
- 실패한 파일만 재처리하는 retry 로직 추가


### 4. 실시간 피드백 및 모니터링 부재

**현재 한계**
- CLI 배치 처리만 지원, 개별 파일 확인 어려움
- 파싱 성공률, 실패 케이스 통계 확인 불가

**개선 방향**
- FastAPI + React 기반 웹 UI 구축
- 실시간 OCR 업로드 → 파싱 결과 즉시 확인
- 대시보드: 성공률 차트, 실패 케이스 상위 10개, 실시간 모니터링

### 5. 확장성 제약

**현재 한계**
- 파일 기반 저장 (CSV/JSON)으로 대용량 데이터 관리 어려움
- 검색/집계 불가 (특정 날짜 범위, 차량번호 필터링)

**개선 방향**
- PostgreSQL 연동으로 수만 건 데이터 관리
- 인덱스 기반 빠른 검색 및 집계 쿼리 (차량별 월간 총량 등)
- Active Learning 파이프라인: 실패 케이스 수집 → 라벨링 → 규칙 자동 생성

---

## 개선 우선순위

1. **단기 (1개월)**: Fuzzy Matching, 병렬 처리, 중량 복구 최적화
2. **중기 (3개월)**: 웹 UI, DB 연동, 차량번호 패턴 확장
3. **장기 (6개월)**: Active Learning, 실시간 모니터링, LLM 통합